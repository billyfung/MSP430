...
void main(void)
{
	...
	//set up timer A0
	TA1CTL = TBSSEL1 + MC1 + TACLR; 										// SMCLK divider = 1; Cont mode
	TA1CCTL1 = CM0 + SCS + CCIS_0 + CAP + CCIE;

	//set up timer A1
	TA0CTL = TBSSEL1 + MC1 + TACLR; 										// SMCLK divider = 1; Cont mode
	TA0CCTL2 = CM0 + SCS + CCIS_0 + CAP + CCIE;


	//PWM output to AIN1_DRV1 & AIN2_DRV1
	P1DIR |= BIT4 + BIT5;// + BIT0 + BIT1;
	P1SEL0 |= BIT4 + BIT5 + BIT1 + BIT2;// + BIT0 + BIT1;
